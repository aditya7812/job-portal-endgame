    ...
    return (
        ...
        <div className="user-settings">
          ...
          <button className="search-buttons card-buttons register" onClick={handlePostJob}>Post Job</button> 
            : 
            currentUserData ? <div className="profileImgTag"><Link to={`users/${currentUser.uid}`}><img src={currentUserData.imageUrl ? currentUserData.imageUrl : logo} alt="" className="actualImg" /></Link></div> :
              <button className="search-buttons card-buttons register" ><Link className="whiteTextTag" to={`users/${currentUser.uid}/update`}>Create Profile</Link></button>
            }
